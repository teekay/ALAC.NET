...
namespace ALACdotNET.Decoder
{
    internal class AlacFile
    {
        ...
        /* stuff from setinfo */
        private int _setinfoMaxSamplesPerFrame; // 0x1000 = 4096
                                               /* max samples per frame? */
        // ReSharper disable once NotAccessedField.Local - we might need it eventually
        ...
        // ReSharper disable once NotAccessedField.Local - we might need it eventually
        private int _setinfo82; // 0x000020e7
                                /* max sample size?? */
        // ReSharper disable once NotAccessedField.Local - we might need it eventually
        private int _setinfo86; // 0x00069fe4
                                /* bit rate (avarge)?? */
        // ReSharper disable once NotAccessedField.Local - we might need it eventually
        ...
        private int CountLeadingZerosExtra(int curbyteParam, int initialZeroes)
        {
            ...
            var curbyte = !initialCondition ? curbyteParam >> 4 : curbyteParam;
            var conditions = new List<(Func<bool> Condition, int Output)>{
                (() => (curbyte & 0x8) != 0, 0),
                (() => (curbyte & 0x4) != 0, 1),
                (() => (curbyte & 0x2) != 0, 2),
                (() => (curbyte & 0x1) != 0, 3),
            ...
            return conditions.First(c => c.Condition()).Output + zeroes;
        }
        ...
        private int CountLeadingZeros(int input)
        {
            ...
            var conditions = new List<(Func<int, bool> ReturnIf, Func<int> SetCurbyteTo)>
            {
                (x => x != 0, () => input >> 16),
                (x => (x & 0xFF) != 0, () => input >> 8),
                (x => (x & 0xFF) != 0, () => input),
            ...
            foreach (var (returnIf, setCurbyteTo) in conditions)
            {
                if (returnIf(curbyte))
                {
        ...
        private int EntropyDecodeValue(int readSampleSize, int k, int riceKmodifierMask)
        {
            ...
            int incrementUntil(int x) => x <= RiceThreshold && Readbit() != 0 ? incrementUntil(x + 1) : x;
            int decodedValue = incrementUntil(0);
            if (decodedValue > RiceThreshold)
            {
                // read the number from the bit stream (raw value), and mask value
                return Readbits(readSampleSize) & (int)(0xffffffff >> (32 - readSampleSize));
            }
            if (k == 1) return decodedValue;
        ...
        private void EntropyRiceDecode(int[] outputBuffer, int outputSize, int readSampleSize, int riceInitialhistory, int riceKmodifier, int riceHistorymult, int riceKmodifierMask)
        {
            ...
            {
                var initialK = 31 - riceKmodifier - CountLeadingZeros((history >> 9) + 3);
                var k = initialK < 0 ? initialK + riceKmodifier : riceKmodifier;
                // note: don't use rice_kmodifier_mask here (set mask to 0xFFFFFFFF)
                var decodedValue = EntropyDecodeValue(readSampleSize, k, 0xFFFFFFFF) + signModifier;
                var almostFinalValue = (decodedValue + 1) / 2;
                outputBuffer[outputCount] = ((decodedValue & 1) != 0) ? almostFinalValue * -1 : almostFinalValue;
                signModifier = 0;
                // update history
                history = decodedValue > 0xFFFF ? 0xFFFF : history + (decodedValue * riceHistorymult) - ((history * riceHistorymult) >> 9);
        ...
        private int EntropyDecodeValue(int readSampleSize, int k, uint v) => EntropyDecodeValue(readSampleSize, k, (int)v);
        ...
        private int[] PredictorDecompressFirAdapt(int[] errorBuffer, int outputSize, int readsamplesize, int[] predictorCoefTable, int predictorCoefNum, int predictorQuantitization)
        {
            ...
            {
                if (outputSize <= 1) return (bufferOut);
            ...
            for (int i = predictorCoefNum + 1; i < outputSize; i++)
            {
                ...
                outval = outval + bufferOut[bufferOutIdx] + errorVal;
                bitsmove = 32 - readsamplesize;
                outval = ((outval << bitsmove) >> bitsmove);
                bufferOut[bufferOutIdx + predictorCoefNum + 1] = outval;
                if (errorVal != 0)
                {
                    Func<int, bool> whileValGt0 = v => v > 0;
                    Func<int, bool> whileValLt0 = v => v < 0;
